...
class TernaryExpressionVisitor(ExpressionVisitor):
    ...
    def visit_literal_expression(self, expression_node: LiteralExpressionNode) -> Callable:
        ...
        if expression_node.operator not in self.comparison_ops:
            ...
            # #We need to convert to boolean logic for filtering.
            # if expression_node.logic_type == "boolean":

            # if expression_node.logic_type == "ternary":
            #     expression_node = TernaryExpressionBuilder.is_true(expression_node)
