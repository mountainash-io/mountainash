"""Narwhals ScalarAggregateExpressionProtocol implementation.

Implements aggregation operations for the Narwhals backend.
"""

from __future__ import annotations

from typing import Any, TYPE_CHECKING

import narwhals as nw

from .base import NarwhalsBaseExpressionSystem

if TYPE_CHECKING:
    from mountainash_expressions.core.expression_protocols.substrait.prtcl_scalar_aggregate import (
        ScalarAggregateExpressionProtocol,
    )

# Type alias for expression type
SupportedExpressions = nw.Expr


class NarwhalsScalarAggregateSystem(NarwhalsBaseExpressionSystem):
    """Narwhals implementation of ScalarAggregateExpressionProtocol.

    Implements aggregation methods:
    - count: Count values in a set
    - count_all: Count all records
    - any_value: Select arbitrary value from group
    """

    def count(
        self,
        x: SupportedExpressions,
        /,
        overflow: Any = None,
    ) -> SupportedExpressions:
        """Count a set of values.

        Counts non-null values.

        Args:
            x: Expression to count.
            overflow: Overflow handling (ignored in Narwhals).

        Returns:
            Count expression.
        """
        return x.count()

    def count_all(
        self,
        overflow: Any = None,
    ) -> SupportedExpressions:
        """Count a set of records (not field referenced).

        Counts all rows including nulls.

        Args:
            overflow: Overflow handling (ignored in Narwhals).

        Returns:
            Count expression.
        """
        return nw.len()

    def any_value(
        self,
        x: SupportedExpressions,
        /,
        ignore_nulls: Any = None,
    ) -> SupportedExpressions:
        """Select an arbitrary value from a group of values.

        Returns the first value in the group.
        If input is empty, returns null.

        Args:
            x: Expression to select from.
            ignore_nulls: Whether to ignore null values.

        Returns:
            First value in group.
        """
        if ignore_nulls:
            return x.drop_nulls().first()
        return x.first()

    # =========================================================================
    # Additional Aggregate Methods (Common Extensions)
    # =========================================================================

    def sum(self, x: SupportedExpressions, /) -> SupportedExpressions:
        """Sum values.

        Args:
            x: Expression to sum.

        Returns:
            Sum expression.
        """
        return x.sum()

    def mean(self, x: SupportedExpressions, /) -> SupportedExpressions:
        """Calculate mean of values.

        Args:
            x: Expression to average.

        Returns:
            Mean expression.
        """
        return x.mean()

    def min(self, x: SupportedExpressions, /) -> SupportedExpressions:
        """Get minimum value.

        Args:
            x: Expression to find minimum.

        Returns:
            Min expression.
        """
        return x.min()

    def max(self, x: SupportedExpressions, /) -> SupportedExpressions:
        """Get maximum value.

        Args:
            x: Expression to find maximum.

        Returns:
            Max expression.
        """
        return x.max()

    def std(self, x: SupportedExpressions, /) -> SupportedExpressions:
        """Calculate standard deviation.

        Args:
            x: Expression to calculate std.

        Returns:
            Standard deviation expression.
        """
        return x.std()

    def var(self, x: SupportedExpressions, /) -> SupportedExpressions:
        """Calculate variance.

        Args:
            x: Expression to calculate variance.

        Returns:
            Variance expression.

        Note:
            Narwhals may not have var. Falls back to std squared.
        """
        # Narwhals doesn't have var - use std squared as approximation
        return x.std().pow(nw.lit(2))

    def median(self, x: SupportedExpressions, /) -> SupportedExpressions:
        """Calculate median.

        Args:
            x: Expression to calculate median.

        Returns:
            Median expression.

        Note:
            Narwhals may not have median. Falls back to mean.
        """
        # Narwhals doesn't have median - fallback to mean
        return x.mean()

    def first(self, x: SupportedExpressions, /) -> SupportedExpressions:
        """Get first value.

        Args:
            x: Expression to get first value.

        Returns:
            First value expression.
        """
        return x.first()

    def last(self, x: SupportedExpressions, /) -> SupportedExpressions:
        """Get last value.

        Args:
            x: Expression to get last value.

        Returns:
            Last value expression.
        """
        return x.last()

    def n_unique(self, x: SupportedExpressions, /) -> SupportedExpressions:
        """Count unique values.

        Args:
            x: Expression to count unique values.

        Returns:
            Unique count expression.
        """
        return x.n_unique()
