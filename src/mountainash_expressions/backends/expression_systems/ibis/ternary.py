"""Ibis ternary logic operations implementation.
...
class IbisTernaryExpressionSystem(IbisBaseExpressionSystem, TernaryExpressionProtocol):
    """
    Ibis implementation of ternary logic operations.

    Implements TernaryExpressionProtocol methods using Ibis expressions.
    ...
    def _ternary_comparison(
        ...
        return ibis.ifelse(
            left_is_unknown | right_is_unknown,
    ...
    def t_xor_parity(self, left: ir.Value, right: ir.Value) -> ir.IntegerValue:
        """
    ...
    def always_true_ternary(self) -> ir.IntegerValue:
        """Return literal TRUE (1)."""
        return ibis.literal(int(T_TRUE))

    def always_false_ternary(self) -> ir.IntegerValue:
        """Return literal FALSE (-1)."""
        return ibis.literal(int(T_FALSE))

    def always_unknown(self) -> ir.IntegerValue:
    ...
    def is_true_ternary(self, operand: ir.Value) -> ir.BooleanValue:
        """TRUE(1) → True, else → False."""
        # Cast to int to avoid Ibis type coercion issues
        return operand == ibis.literal(int(T_TRUE))

    def is_false_ternary(self, operand: ir.Value) -> ir.BooleanValue:
        """FALSE(-1) → True, else → False."""
        return operand == ibis.literal(int(T_FALSE))

    def is_unknown(self, operand: ir.Value) -> ir.BooleanValue:
        """UNKNOWN(0) → True, else → False."""
        return operand == ibis.literal(int(T_UNKNOWN))

    def is_known(self, operand: ir.Value) -> ir.BooleanValue:
        """TRUE or FALSE → True, UNKNOWN → False."""
        return operand != ibis.literal(int(T_UNKNOWN))

    def maybe_true(self, operand: ir.Value) -> ir.BooleanValue:
        """TRUE or UNKNOWN → True, FALSE → False."""
        return operand >= ibis.literal(int(T_UNKNOWN))

    def maybe_false(self, operand: ir.Value) -> ir.BooleanValue:
    ...
    def to_ternary(self, operand: ir.Value) -> ir.IntegerValue:
        """True → 1, False → -1."""
