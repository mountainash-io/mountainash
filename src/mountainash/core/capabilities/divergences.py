...
def _all() -> tuple[DivergenceFact, ...]:
    ...
    return (
        ...
        DivergenceFact(
            id="NW-WIN-01",
            kind=DivergenceKind.ENGINE_LENIENCY,
            operation_keys=(FK_WIN.DIFF,),
            backends=("narwhals-lazy",),
            summary="narwhals-lazy rejects diff(): an order-dependent window expression is not permitted on a LazyFrame (InvalidOperationError)",
            impact="col().diff() raises on narwhals-lazy; eager narwhals and polars compute consecutive differences",
        ...
        DivergenceFact(
            ...
            summary="ibis-polars has no translation rule for the diff/cumulative WindowFunction (OperationNotDefinedError)",
            impact="col().diff() raises on ibis-polars; ibis-duckdb/ibis-sqlite and polars/narwhals compute it",
