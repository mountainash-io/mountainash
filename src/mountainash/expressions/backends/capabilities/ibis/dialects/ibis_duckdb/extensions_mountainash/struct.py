...
SEGMENT = CapabilitySegment(domain=Domain.STRUCT, capabilities=(CapabilityAssertion(key=CapabilityKey(operation=FKEY_MOUNTAINASH_SCALAR_STRUCT.CAST, subject='failure_behavior', selector=Selector(kind='predicate', value=Predicate(clauses=(Clause(path='failure_behavior', op=ClauseOp.EQ, operand='null'),)))), level=CapabilityLevel.UNSUPPORTED, since='2026-08-24', origins=(LocalOrigin(entry='capabilities[0]'), SourceOrigin(module='mountainash.expressions.backends.capabilities.struct', scope=Scope(backend=CONST_BACKEND.IBIS, applicability=Dialect(name='ibis-duckdb')), source=FactSource.MOUNTAINASH, domain=Domain.STRUCT, entry='DECLARATIONS[2].facts[1]', captured=CapturedAddress(repository='mountainash', path='src/mountainash/expressions/backends/capabilities/struct.py', entry='DECLARATIONS[2].facts[1]', revision='80dc283a1eb65c33b0b3bae946b3a8f5305cd66a')),), message='This backend cannot execute STRUCT.CAST for the requested failure behavior'),), evidence_refs=(CapturedAddress(repository='mountainash', path='src/mountainash/expressions/backends/capabilities/struct.py', entry='DECLARATIONS[2]', revision='80dc283a1eb65c33b0b3bae946b3a8f5305cd66a'),))
