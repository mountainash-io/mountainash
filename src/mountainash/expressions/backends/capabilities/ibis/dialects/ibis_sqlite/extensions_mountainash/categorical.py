...
SEGMENT = CapabilitySegment(domain=Domain.CATEGORICAL, capabilities=(CapabilityAssertion(key=CapabilityKey(operation=FKEY_MOUNTAINASH_SCALAR_CATEGORICAL.CAST, subject='value_type', selector=Selector(kind='predicate', value=Predicate(clauses=(Clause(path='value_type', op=ClauseOp.EQ, operand='integer'),)))), level=CapabilityLevel.UNSUPPORTED, since='2026-08-24', origins=(LocalOrigin(entry='capabilities[0]'), SourceOrigin(module='mountainash.expressions.backends.capabilities.categorical', scope=Scope(backend=CONST_BACKEND.IBIS, applicability=Dialect(name='ibis-sqlite')), source=FactSource.MOUNTAINASH, domain=Domain.CATEGORICAL, entry='DECLARATIONS[2].facts[0]', captured=CapturedAddress(repository='mountainash', path='src/mountainash/expressions/backends/capabilities/categorical.py', entry='DECLARATIONS[2].facts[0]', revision='80dc283a1eb65c33b0b3bae946b3a8f5305cd66a')),), message='This backend cannot execute CATEGORICAL.CAST for the requested value type and failure behavior'),), evidence_refs=(CapturedAddress(repository='mountainash', path='src/mountainash/expressions/backends/capabilities/categorical.py', entry='DECLARATIONS[2]', revision='80dc283a1eb65c33b0b3bae946b3a8f5305cd66a'),))
