...
SEGMENT = CapabilitySegment(domain=Domain.TERNARY, capabilities=(CapabilityAssertion(key=CapabilityKey(operation=FKEY_MOUNTAINASH_SCALAR_TERNARY.COLLECT_VALUES, subject='*'), level=CapabilityLevel.POLYMORPHIC, since='2026-07-05', origins=(LocalOrigin(entry='capabilities[0]'), SourceOrigin(module='mountainash.expressions.backends.capabilities.polymorphic', scope=Scope(backend=CONST_BACKEND.IBIS, applicability=FamilyWide()), source=FactSource.MOUNTAINASH, domain=Domain.TERNARY, entry='DECLARATIONS[4].facts[0]', captured=CapturedAddress(repository='mountainash', path='src/mountainash/expressions/backends/capabilities/polymorphic.py', entry='DECLARATIONS[4].facts[0]', revision='80dc283a1eb65c33b0b3bae946b3a8f5305cd66a')),), message='literal collections unwrap to raw values; expressions compile through (LIST-wrapper marker)', probe_exempt='polymorphic — both paths supported by design'),), evidence_refs=(CapturedAddress(repository='mountainash', path='src/mountainash/expressions/backends/capabilities/polymorphic.py', entry='DECLARATIONS[4]', revision='80dc283a1eb65c33b0b3bae946b3a8f5305cd66a'),))
