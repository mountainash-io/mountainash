...
def import_polars() -> Any:
    ...
    return require_module("polars", "pip install polars==1.16.0")
...
def import_ibis() -> Any:
    """Lazy import of ibis."""
    return require_module("ibis", "pip install 'ibis-framework[polars,sqlite,duckdb]==11.0.0'")

def import_ibis_expr_types() -> Any:
    """Lazy import of ibis.expr.types."""
    if not require_module("ibis", "pip install 'ibis-framework[polars,sqlite,duckdb]==11.0.0'"):
        return None
    return require_module("ibis.expr.types", "pip install 'ibis-framework[polars,sqlite,duckdb]==11.0.0'")

def import_ibis_expr_ops() -> Any:
    """Lazy import of ibis.expr.operations."""
    if not require_module("ibis", "pip install 'ibis-framework[polars,sqlite,duckdb]==10.4.0'"):
        return None
    return require_module("ibis.expr.operations", "pip install 'ibis-framework[polars,sqlite,duckdb]==11.0.0'")
...
INSTALL_COMMANDS = {
    ...
    "polars": "pip install polars==1.16.0",
    "pyarrow": "pip install pyarrow==17.0.0",
    "ibis": "pip install 'ibis-framework[polars,sqlite,duckdb]==10.4.0'",
    "ibis.expr.types": "pip install 'ibis-framework[polars,sqlite,duckdb]==10.4.0'",
...
def get_missing_dependency_error(backend: str, operation: str = "") -> str:
    ...
    return (
        ...
        f"Or install all optional dependencies with: pip install 'mountainash-dataframes[all]'"
    )
