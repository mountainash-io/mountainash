"""
TypeSpec Extractors - Extract TypeSpec from Various Sources

Provides functions to extract TypeSpec from:
- Python dataclasses
...
# Cache for derived schemas: Type -> TypeSpec
# Classes don't change at runtime, so simple caching without invalidation
_DATACLASS_SCHEMA_CACHE: Dict[Type, TypeSpec] = {}
_PYDANTIC_SCHEMA_CACHE: Dict[Type, TypeSpec] = {}
...
def extract_from_dataclass(
    ...
    **metadata
    ...
    """
    Extract TypeSpec from a Python dataclass.
    ...
    Returns:
        TypeSpec extracted from dataclass
    ...
    Example:
        ...
        >>> schema = extract_from_dataclass(User)
        >>> schema.field_names
        ['id', 'name', 'email']
    """
...
def extract_from_pydantic(
    model_type: Type['BaseModel'],
    **metadata
) -> TypeSpec:
    """
    Extract TypeSpec from a Pydantic model.
    ...
    Returns:
        TypeSpec extracted from Pydantic model
    ...
    Example:
        ...
        >>> schema = extract_from_pydantic(User)
    """
    try:
        import pydantic
    except ImportError:
        raise ImportError("pydantic is required for extract_from_pydantic()")
...
def _from_pydantic_v1(model_type: Type['BaseModel'], **metadata) -> TypeSpec:
...
def _from_pydantic_v2(model_type: Type['BaseModel'], **metadata) -> TypeSpec:
...
def extract_from_dataframe(
    ...
    **metadata
    ...
    """
    Extract TypeSpec from any DataFrame backend.
    ...
    Returns:
        TypeSpec extracted from DataFrame

    Example:
        >>> import polars as pl
        >>> df = pl.DataFrame({"id": [1, 2], "name": ["Alice", "Bob"]})
        >>> schema = extract_from_dataframe(df)
        >>> schema.field_names
        ['id', 'name']
    """
    # Detect backend
...
def _from_pandas(df: 'pd.DataFrame', preserve_backend_types: bool, **metadata) -> TypeSpec:
...
def extract_schema_from_dataframe(
    ...
    """
    Auto-derive TypeSpec from DataFrame.
    ...
    Returns:
        TypeSpec matching DataFrame structure
    ...
    Example:
        ...
        ['id', 'name']
    """
    return extract_from_dataframe(df, preserve_backend_types=include_backend_types)
...
def extract_schema_from_dataclass(
    ...
    """
    Derive TypeSpec from dataclass definition with caching.
    ...
    Returns:
        TypeSpec matching dataclass structure
    ...
    # Extract schema
    schema = extract_from_dataclass(dataclass_type, preserve_optional=True)
...
def extract_schema_from_pydantic(
    ...
    """
    Derive TypeSpec from Pydantic model with caching.
    ...
    Returns:
        TypeSpec matching model structure
    ...
    # Extract schema
    schema = extract_from_pydantic(model_class)
