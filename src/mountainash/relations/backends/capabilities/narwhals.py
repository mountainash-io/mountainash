"""Import-safe Narwhals relation-backend capability declarations.

Migrated from ``NarwhalsBaseRelationSystem.CAPABILITIES`` in
...
NARWHALS_REL_CAPABILITIES: tuple[CapabilityFact, ...] = (
    CapabilityFact(
...
DECLARATIONS = (
    CapabilityDeclaration(
        backend=CONST_BACKEND.NARWHALS, domain=Domain.RELATION,
        source=FactSource.MOUNTAINASH, facts=NARWHALS_REL_CAPABILITIES,
        evidence=_EVIDENCE,
    ),
)
