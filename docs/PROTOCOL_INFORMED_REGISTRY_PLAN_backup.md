...
class FunctionDef:
    ...
    # The protocol that defines the method signature
    protocol: Type[Protocol]  # e.g., BooleanExpressionProtocol

    # The method name on the protocol (for introspection)
    method: str  # e.g., "eq"
...
def register_all_functions():
    ...
    # String functions - note protocol method has typed signature
    FunctionRegistry.register(FunctionDef(
        func=SUBSTRAIT_STRING.CONTAINS,
        protocol=StringExpressionProtocol,
        method="str_contains",  # Protocol method: str_contains(operand, substring: str)
...
class UnifiedExpressionVisitor:
    ...
    def visit_scalar_function(self, node: ScalarFunctionNode):
        ...
        # Get backend method and call
        method = getattr(self.backend, func_def.method)
...
5. **Substrait alignment** - ENUM values ARE Substrait function names
6. **Protocol contracts preserved** - Signatures define the interface
7. **Extensible** - Add new ENUMs and registry entries for new functions
