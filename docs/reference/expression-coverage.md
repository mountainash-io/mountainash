...
| --- | --- | --- | --- | --- | --- | --- |
| polars | 219 | 70 | 52 | 0 | 0 | 341 |
| narwhals | 104 | 146 | 91 | 0 | 0 | 341 |
| ibis | 102 | 148 | 91 | 0 | 0 | 341 |
...
| `ALL` | ✓ | ✓ audited | ✓ audited |
| `ANY` | ✓ | ✓ audited | ✓ audited |
| `ARG_MAX` | ✓ | ✓ audited | ✓ audited |
| `ARG_MIN` | ✓ | ✓ audited | ✓ audited |
| `CAST_ITEMS` | ✓ | ◐ partial (1 params, 2 option-selectors, 0 metadata-selectors, 0 value-classes, 3 dialects) | ◐ partial (1 params, 2 option-selectors, 0 metadata-selectors, 0 value-classes, 3 dialects) |
| `CONCAT` | ✓ | ✓ audited | ✓ audited |
| `CONTAINS` | ✓ | ◐ partial (1 params, 0 option-selectors, 0 metadata-selectors, 0 value-classes, 1 dialects) ⚠ runtime | ✓ audited |
| `COUNT_MATCHES` | ✓ | ✓ audited | ✓ audited |
| `DIFF` | ✓ | ✓ audited | ✓ audited |
| `DROP_NULLS` | ✓ | ✓ audited | ✓ audited |
| `EXPLODE` | ✓ | ✓ audited | ✓ audited |
| `FILTER` | ✓ | ✓ audited | ✓ audited |
| `GATHER` | ✓ | ✓ audited | ✓ audited |
| `GATHER_EVERY` | ✓ | ✓ audited | ✓ audited |
| `GET` | ✓ | ◐ partial (1 params, 0 option-selectors, 0 metadata-selectors, 0 value-classes, 1 dialects) ⚠ runtime | ✓ audited |
| `HEAD` | ✓ | ✓ audited | ✓ audited |
| `ITEM` | ✓ | ✓ audited | ✓ audited |
| `JOIN` | ✓ | ✓ audited | ✓ audited |
| `LEN` | ✓ | ✓ audited | ✓ audited |
| `MAX` | ✓ | ✓ audited | ✓ audited |
| `MEAN` | ✓ | ✓ audited | ✓ audited |
| `MEDIAN` | ✓ | ✓ audited | ✓ audited |
| `MIN` | ✓ | ✓ audited | ✓ audited |
| `N_UNIQUE` | ✓ | ✓ audited | ✓ audited |
| `PARSE` | ✓ | ◐ partial (2 params, 2 option-selectors, 0 metadata-selectors, 0 value-classes, 3 dialects) ⚠ runtime | ◐ partial (2 params, 6 option-selectors, 0 metadata-selectors, 0 value-classes, 3 dialects) · unsupported on ibis-sqlite |
| `REVERSE` | ✓ | ✓ audited | ✓ audited |
| `SAMPLE` | ✓ | ✓ audited | ✓ audited |
| `SET_DIFFERENCE` | ✓ | ✓ audited | ✓ audited |
| `SET_INTERSECTION` | ✓ | ✓ audited | ✓ audited |
| `SET_SYMMETRIC_DIFFERENCE` | ✓ | ✓ audited | ✓ audited |
| `SET_UNION` | ✓ | ✓ audited | ✓ audited |
| `SHIFT` | ✓ | ✓ audited | ✓ audited |
| `SLICE` | ✓ | ✓ audited | ✓ audited |
| `SORT` | ✓ | ✓ audited | ✓ audited |
| `STD` | ✓ | ✓ audited | ✓ audited |
| `SUM` | ✓ | ✓ audited | ✓ audited |
| `TAIL` | ✓ | ✓ audited | ✓ audited |
| `TO_ARRAY` | ✓ | ✓ audited | ✓ audited |
| `TO_STRUCT` | ✓ | ✓ audited | ✓ audited |
| `T_CONTAINS` | ✓ | ◐ partial (1 params, 0 option-selectors, 0 metadata-selectors, 0 value-classes, 1 dialects) ⚠ runtime | ✓ audited |
| `UNIQUE` | ✓ | ✓ audited | ✓ audited |
| `VAR` | ✓ | ✓ audited | ✓ audited |
...
| IB-DT-09 | semantics | ibis-duckdb, ibis-sqlite | `NOW` | now() returns naive UTC query-time timestamps on ibis-duckdb and ibis-sqlite | On non-UTC hosts, now() differs from the local naive datetime snapshot used by Polars, Narwhals and ibis-polars | Account for UTC query-time semantics, or use Polars, Narwhals or ibis-polars for local datetime snapshots | IB-DT-09 | 2026-07-05 |
...
| IB-LIST-01 | engine_leniency | ibis | `GATHER_EVERY`, `ARG_MIN`, `ARG_MAX`, `N_UNIQUE`, `COUNT_MATCHES`, `DROP_NULLS`, `SET_DIFFERENCE`, `STD`, `VAR`, `SHIFT`, `DIFF`, `REVERSE`, `SLICE`, `HEAD`, `TAIL`, `MEDIAN` | Ibis lacks native array operations for these list ops; the mountainash ibis expression system raises BackendCapabilityError (unenriched) | list.gather_every/arg_min/arg_max/n_unique/count_matches/drop_nulls/set_difference/std/var/shift/diff/reverse/slice/head/tail/median raise on ibis backends | Use a Polars backend for these list operations | IB-LIST-01 | 2026-08-06 |
...
| IB-REL-18 | engine_leniency | ibis-duckdb | — | DuckDB does not guarantee physical row order for a filtered scan; some OR-of-AND boolean predicates compile to a query plan that reorders matching rows relative to the left input, even though polars/narwhals/ibis-polars/ibis-sqlite all preserve input row order through filter | Relation.filter() with certain compound boolean predicates may return rows in a different order than the input on ibis-duckdb; matched VALUES are always correct, only row position differs | Use polars, narwhals, ibis-polars, or ibis-sqlite if exact input row order must be preserved through filter, or add an explicit .sort() after filter | — | 2026-09-03 |
...
| MA-CONF-04 | engine_leniency | ibis-sqlite | — | ibis-sqlite cannot construct a native struct-typed table | conform() with a native OBJECT source raises UnsupportedBackendType on ibis-sqlite; other supported native structured backends execute it | Use a non-SQLite Ibis backend or a Polars-backed backend for native struct conform | — | 2026-08-18 |
| MA-CONF-06 | engine_leniency | narwhals-polars | — | Polars Object dtype has no defined Arrow representation; Narwhals' to_arrow() serializes raw object pointers instead of the underlying value | an opaque-carrier structured field (no schema evidence, already-native Python container) fails to decode through logical egress when the source is a Narwhals-wrapped Polars frame | Use a bare Polars relation (not Narwhals-wrapped) or a pandas/narwhals-pandas source for an opaque structured field; a JSON-text or schema-proven native LIST/STRUCT carrier is unaffected | — | 2026-08-28 |
...
| MA-STR-01 | engine_leniency | pandas, ibis-polars | `CONTAINS` | str.contains with a columnar (per-row) literal pattern is unsupported on pandas (enriched BackendCapabilityError, not registry-resolvable) and ibis-polars (UnsupportedArgumentError) | col.str.contains(other_col) raises on pandas and ibis-polars; polars and ibis-duckdb/ibis-sqlite compute the per-row substring test | Use a polars or ibis SQL backend for columnar substring patterns | MA-STR-01 | 2026-08-06 |
...
| NW-AGG-03 | engine_leniency | narwhals-lazy | `MODE`, `ANY_VALUE` | narwhals-lazy rejects mode()/any_value(): order-dependent/length-changing aggregates are not permitted on a LazyFrame (InvalidOperationError) | mode() and any_value() raise on narwhals-lazy; eager narwhals and other backends compute them | Use an eager backend for mode()/any_value() | NW-AGG-03 | 2026-08-06 |
...
| NW-DT-03 | engine_leniency | narwhals | `TIME` | Narwhals does not support dt.time() — NotImplementedError on all narwhals backends | dt.time() raises on narwhals-polars/narwhals-pandas/narwhals-lazy; polars and ibis-duckdb/ibis-polars compute it | Use a polars or ibis backend for dt.time() | NW-DT-03 | 2026-08-06 |
...
| NW-DT-06 | engine_leniency | pandas, narwhals | `EXTRACT_WEEK` | pandas and narwhals lack ISO week_of_year; the mountainash expression system raises BackendCapabilityError (unenriched) | dt.week_of_year() raises on pandas and all narwhals backends; polars and ibis compute it | Use a polars or ibis backend for ISO week extraction | NW-DT-06 | 2026-08-06 |
| NW-DT-07 | engine_leniency | narwhals-pandas | `DATE` | narwhals-pandas dt.date() raises NotImplementedError | dt.date() raises on narwhals-pandas; narwhals-polars, polars and ibis compute it | Use narwhals-polars, polars, or an ibis backend for dt.date() | NW-DT-07 | 2026-08-06 |
| NW-LIST-05 | engine_leniency | narwhals | `GATHER_EVERY`, `ARG_MIN`, `ARG_MAX`, `ALL`, `ANY`, `N_UNIQUE`, `COUNT_MATCHES`, `DROP_NULLS`, `SET_UNION`, `SET_INTERSECTION`, `SET_DIFFERENCE`, `STD`, `VAR`, `SHIFT`, `DIFF`, `CONCAT`, `JOIN`, `REVERSE`, `SLICE`, `HEAD`, `TAIL`, `EXPLODE` | Narwhals lacks native list operations; the mountainash narwhals expression system raises BackendCapabilityError (unenriched) for these list ops | list.gather_every/arg_min/arg_max/all/any/n_unique/count_matches/drop_nulls/set_union/set_intersection/set_difference/std/var/shift/diff/concat/join/reverse/slice/head/tail/explode raise on narwhals backends | Use a Polars or Ibis backend for these list operations | NW-LIST-05 | 2026-08-06 |
| NW-MATH-01 | engine_leniency | pandas, narwhals | — | pandas and narwhals lack tan(), so cot() (computed as 1/tan) raises NotImplementedError | ma.col(x).cot() raises on pandas and all narwhals backends; polars and ibis compute it | Use a polars or ibis backend for cot() | NW-MATH-01 | 2026-08-06 |
...
| NW-WIN-03 | engine_leniency | narwhals-lazy | `DIFF`, `CUM_SUM`, `CUM_MAX`, `CUM_MIN`, `CUM_COUNT`, `CUM_PROD`, `LEAD`, `LAG`, `FIRST_VALUE`, `LAST_VALUE` | narwhals-lazy rejects order-dependent window expressions (diff/cumulative/lead/lag/shift/first_value/last_value) on a LazyFrame (InvalidOperationError) | order-dependent window ops (diff/cum_*/lead/lag/shift/first_value/last_value) raise on narwhals-lazy; eager narwhals and polars compute them | Use an eager backend, or establish an explicit order before the lazy diff | NW-WIN-03 | 2026-08-06 |
| NW-WIN-04 | engine_leniency | narwhals, pandas | `PERCENT_RANK`, `CUME_DIST`, `NTH_VALUE`, `DIFF` | narwhals backends raise NotImplementedError for percent_rank()/cume_dist()/nth_value() and diff(n>1) | percent_rank/cume_dist/nth_value and multi-step diff raise on narwhals/pandas; ibis-duckdb/sqlite compute them (polars too, except nth_value — see PL-WIN-01) | Use a polars or ibis SQL backend for these window ops | NW-WIN-04 | 2026-08-06 |
