"""
Shared fixtures for mountainash.typespec tests.

Provides:
- Sample DataFrames for all backends (Polars, Pandas, PyArrow, Narwhals, Ibis)
- Sample TypeSpec fixtures (simple and full)
...
def simple_table_schema():
    """Minimal TypeSpec with two fields."""
    return TypeSpec.from_simple_dict({"id": "integer", "name": "string"})


@pytest.fixture
def full_table_schema():
    """TypeSpec with all common types, constraints, and metadata."""
    fields = [
        FieldSpec(
            name="employee_id",
...
@pytest.fixture
def source_schema():
    """Source TypeSpec with user_ prefix field names."""
...
@pytest.fixture
def target_schema():
    """Target TypeSpec with short field names."""
    return TypeSpec.from_simple_dict({
        "id": "integer",
        "name": "string",
        "email": "string",
