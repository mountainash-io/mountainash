...
class TestClip:
    def test_clip_both_bounds(self, backend_name, backend_factory, collect_expr):
    ...
    def test_clip_lower_only(self, backend_name, backend_factory, collect_expr):
        data = {"val": [1, 5, 10]}
        df = backend_factory.create(data, backend_name)
    ...
    def test_clip_upper_only(self, backend_name, backend_factory, collect_expr):
        data = {"val": [1, 5, 10]}
        df = backend_factory.create(data, backend_name)
    ...
    def test_clip_no_bounds(self, backend_name, backend_factory, collect_expr):
        data = {"val": [1, 5, 10]}
        df = backend_factory.create(data, backend_name)
...
class TestIsNotNan:
    def test_is_not_nan_basic(self, backend_name, backend_factory, collect_expr):
...
class TestFillNan:
    def test_fill_nan_with_value(self, backend_name, backend_factory, collect_expr):
        data = {"val": [1.0, float("nan"), 3.0, float("nan"), 5.0]}
        df = backend_factory.create(data, backend_name)
