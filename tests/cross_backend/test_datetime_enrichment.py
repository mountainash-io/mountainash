...
class TestDateExtraction:
    def test_date(self, backend_name, backend_factory, select_and_extract):
...
class TestTimeExtraction:
    def test_time(self, backend_name, backend_factory, select_and_extract):
        data = {"ts": [datetime(2024, 3, 15, 10, 30), datetime(2024, 7, 20, 14, 0)]}
        df = backend_factory.create(data, backend_name)
...
class TestMonthStart:
    def test_month_start(self, backend_name, backend_factory, select_and_extract):
        data = {"ts": [datetime(2024, 3, 15, 10, 30), datetime(2024, 7, 20, 14, 0)]}
        df = backend_factory.create(data, backend_name)
...
class TestMonthEnd:
    def test_month_end(self, backend_name, backend_factory, select_and_extract):
...
class TestDaysInMonth:
    def test_days_in_month(self, backend_name, backend_factory, select_and_extract):
        data = {"ts": [datetime(2024, 2, 15), datetime(2024, 3, 15)]}
        df = backend_factory.create(data, backend_name)
