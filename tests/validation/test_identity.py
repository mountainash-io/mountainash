...
class TestKeyedValidation:
    def _rel(self, data):
        return ma.relation(pl.DataFrame(data))

    def test_valid_key_returns_zero_diagnostics(self):
        rel = self._rel({"id": [1, 2, 3], "v": [10, 20, 30]})
        diags = validate_keyed_identity(rel, RowIdentity("keyed", ("id",)))
        assert diags == {"null_key_rows": 0, "duplicate_key_tuples": 0}
    ...
    def test_null_key_raises_by_default(self):
        rel = self._rel({"id": [1, None, 3], "v": [1, 2, 3]})
        with pytest.raises(IdentityInvalidError):
            validate_keyed_identity(rel, RowIdentity("keyed", ("id",)))

    def test_duplicate_key_raises_by_default(self):
        rel = self._rel({"id": [1, 1, 3], "v": [1, 2, 3]})
        with pytest.raises(IdentityInvalidError):
            validate_keyed_identity(rel, RowIdentity("keyed", ("id",)))

    def test_allow_imperfect_key_downgrades_to_diagnostics(self):
        rel = self._rel({"id": [1, 1, None], "v": [1, 2, 3]})
        diags = validate_keyed_identity(
            rel, RowIdentity("keyed", ("id",)), allow_imperfect_key=True
        )
        assert diags["null_key_rows"] == 1
        assert diags["duplicate_key_tuples"] == 1
