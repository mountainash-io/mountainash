"""
Tests for mountainash.schema.config.extractors
...
class TestExtractFromPolars:
    ...
    def test_returns_table_schema(self, polars_mixed_df):
        schema = extract_schema_from_dataframe(polars_mixed_df)
...
class TestExtractFromPandas:
    ...
    def test_returns_table_schema(self, pandas_df):
        schema = extract_schema_from_dataframe(pandas_df)
        assert isinstance(schema, TableSchema)
...
class TestExtractFromPyArrow:
    ...
    def test_returns_table_schema(self, pyarrow_table):
        schema = extract_schema_from_dataframe(pyarrow_table)
        assert isinstance(schema, TableSchema)
...
class TestExtractFromNarwhals:
    ...
    def test_returns_table_schema(self, narwhals_df):
        schema = extract_schema_from_dataframe(narwhals_df)
        assert isinstance(schema, TableSchema)
...
class TestExtractFromIbis:
    ...
    def test_returns_table_schema(self, ibis_table):
        schema = extract_schema_from_dataframe(ibis_table)
        assert isinstance(schema, TableSchema)
...
class TestExtractFromDataclass:
    ...
    def test_optional_fields_present(self):
        schema = from_dataclass(OptionalFields)
    ...
    def test_returns_table_schema(self):
        schema = from_dataclass(SimpleTypes)
        assert isinstance(schema, TableSchema)
...
class TestExtractFromPydantic:
    ...
    def test_optional_field_present(self, optional_model):
        schema = from_pydantic(optional_model)
        assert "email" in schema.field_names
    ...
    def test_returns_table_schema(self, basic_model):
        schema = from_pydantic(basic_model)
        assert isinstance(schema, TableSchema)
...
class TestFuzzyMatching:
    """Tests for build_schema_config_with_fuzzy_matching."""

    def test_exact_match_no_renames(self):
        """Exact match columns should produce no rename transforms."""
        ...
        # For exact matches, no rename should be needed
        for col_spec in config.columns.values():
            assert "rename" not in col_spec

    def test_fuzzy_match_produces_rename(self):
        """'user_name' -> 'username' should produce a rename mapping at low threshold."""
        source = TableSchema.from_simple_dict({"user_name": "string"})
        target = TableSchema.from_simple_dict({"username": "string"})
        config = build_schema_config_with_fuzzy_matching(source, target, fuzzy_match_threshold=0.5)
        # Should detect the fuzzy match and add a rename
        assert "user_name" in config.columns
        assert config.columns["user_name"].get("rename") == "username"

    def test_high_threshold_misses_weak_match(self):
        """'usr_nm' -> 'user_name' should not match at 0.95 threshold."""
        ...
        # With very high threshold, the weak fuzzy match should be skipped
        # Either not in columns or no rename mapping
        if "usr_nm" in config.columns:
            assert "rename" not in config.columns["usr_nm"]
